...
"""
TODO (hinten anstellen nach der Eval - und dann schauen, ob es das tatsächlich besser macht!):


Kopplungs Werte nicht nach min/max/median normalisieren, sondern nach dem Histogramm (rank-based): der Wert zwischen 0 und 1 sollte proportional zum rang in der sortierten Liste sein. 0 und 1 bleiben weiterhin die extremfälle (beachte hier wenn viele kanten den gleichen Wert haben!), aber alles dazwischen ist gleichmäßiger verteilt. Das sollte gut die powerlaw Verteilung auflösen und alle Ausreißer ähnlich nah an 0.99 bringen 
...
tatsächlich statistisch-korrektes Bayesian updating statt arbiträren support values?
